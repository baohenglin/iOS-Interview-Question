...
Runtime 对注册的类，会进行布局，会将 weak 对象会入一个 hash 表中。用 weak 指向的对象的内存地址作为 key，当这个对象的引用计数为 0 时，会调用 dealloc 函数。假如 weak 指向的对象内存地址是 a，那么就会以 a 作为 key，在这个 weak 表中进行搜索，查找到所有以 a 为键的 weak 对象，并将查找到的这些 weak 对象都置为 nil。
