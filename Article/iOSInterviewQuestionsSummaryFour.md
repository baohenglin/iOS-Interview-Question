...
* (3)尽量把View设置为完全不透明。如果有透明的Views，应该把opaque(不透明)属性设置为YES，这样可以减小资源消耗，降低内存开销。opaque属性给渲染系统提供了一个如何处理这个view的提示。如果设为 YES， 渲染系统就认为这个view是完全不透明的，这使得渲染系统可以优化一些渲染过程进而提高性能。如果设置为NO，渲染系统正常地和其它内容组成这个View。
